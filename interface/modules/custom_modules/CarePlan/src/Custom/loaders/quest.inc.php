...
if ($form_action == 4) { // load profiles
	...
	// Load category group ids (procedure and profile)
	$groups = sqlQuery( "SELECT procedure_type_id FROM procedure_type WHERE parent = ? AND procedure_type LIKE 'grp' AND name LIKE 'Profiles' ", array (
			$form_group 
	));
	$groupid = $groups['procedure_type_id'];
