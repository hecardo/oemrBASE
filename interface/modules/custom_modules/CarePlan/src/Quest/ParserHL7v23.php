...
class ParserHL7v23 {
	...
	function getRequest() {
		...
		$message->account = $this->MSH['receiving_facility'];
...
//		$message->lab_status = $this->PID['accounting'][5];
		...
		$message->control_id = $this->ORC['filler_order_number'];
		$message->lab_status = '';
/*
 ...
 */
		for ($i = 0; $i < count($map); $i++) {
			...
			while ($item['type'] == 'OBR') {
				...
				$report->control_id = $obr_data['filler_order_number'];
				if (!$message->control_id) $message->control_id = $obr_data['filler_order_number'];
				...
				// get lab data
				if ($obr_data['lab_data']) {
					if (is_array($obr_data['lab_data'])) {
						$lab_code = $obr_data['lab_data'][0];
						$lab_name = $obr_data['lab_data'][1];
						$lab_director = $obr_data['lab_data'][6];
						
						$lab_address = '';
						if ($obr_data['lab_data'][2]) {
							$lab_address = array();
							$lab_address[] = $obr_data['lab_data'][2]; // street
							$lab_address[] = '';
							$lab_address[] = $obr_data['lab_data'][3]; // city
							$lab_address[] = $obr_data['lab_data'][4]; // state
							$lab_address[] = $obr_data['lab_data'][5]; // zip
						}
