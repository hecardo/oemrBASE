...
class LabOrderItem {
	 ...
	 *
	 * @param string $proc_order_id order number
	 * @param int $seq $proc_order_seq order item sequence
	...
	public function __construct($proc_order_id = false, $proc_order_seq = false) {
		...
		if ($data = sqlFetchArray($results)) {
			...
			throw new \Exception('mdtsLabOrderItem::_construct - no procedure order item record with key ('.$proc_order_id.' '.$proc_order_seq.').');
		}
	 ...
	 */
	public static function getBillFee($pricelevel = 'standard') {
	...
	public static function fetchItemList($proc_order_id = false) {
		...
		$objectList = array();
		while ($data = sqlFetchArray($results)) {
			$objectList[] = new LabOrderItem($proc_order_id,$data['procedure_order_seq']);
		}
