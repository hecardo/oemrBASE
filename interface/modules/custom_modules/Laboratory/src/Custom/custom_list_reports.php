...
require_once("$include_root/orders/receive_hl7_results.inc.php");
require_once("$include_root/orders/gen_hl7_order.inc.php");
...
if (!$thisauth) {
    echo (new TwigContainer(null, $GLOBALS['kernel']))->getTwig()->render('core/unauthorized.html.twig', ['pageTitle' => xl("Procedure Processing")]);
    exit;
}
...
<title><?php echo xlt('Procedure Processing'); ?></title>
...
<form class="form-inline" method='post' action='custom_list_reports.php' enctype='multipart/form-data'>
    <div class="container">
            ...
            <div class="form-group">
                <div class="input-group-btn input-group-append">
                    <button class='btn btn-primary' name='form_process_labs'
                        title="Click to process pending results from selected Processors."
                    ...
                    <select name='form_lab_id' id='form_lab_id' class='form-control'>
                        <option value="0"><?php echo xlt('All Processors'); ?></option>
                        <?php
//CUSTOM                        $ppres = sqlStatement("SELECT ppid, name, npi FROM procedure_providers ORDER BY name, ppid");
                        $ppres = sqlStatement("SELECT ppid, name, npi FROM procedure_providers WHERE NULLIF(type,'') IS NULL ORDER BY name, ppid");
                ...
                <div class="form-group">
                    ...
                    <input name='form_max_results' id='form_max_results' class='form-control'
                        ...
                        <span class="input-group-text"><?php echo xlt('Results Per Processor'); ?></span>
                    </div>
    ...
    <div class="form-row my-2">
        ...
        <div class="col-md">
            <select name='form_lab_search' id='form_lab_search' class='form-control'>
                <option value="0"><?php echo xlt('All Processors'); ?></option>
                <?php
//CUSTOM                $ppres = sqlStatement("SELECT ppid, name FROM procedure_providers ORDER BY name, ppid");
                $ppres = sqlStatement("SELECT ppid, name FROM procedure_providers WHERE NULLIF(type,'') IS NULL ORDER BY name, ppid");
                while ($pprow = sqlFetchArray($ppres)) {
                    echo "<option value='" . attr($pprow['ppid']) . "'";
