...
if (! AclMain::aclCheckCore('acct', 'rep')) {
    echo (new TwigContainer(null, $GLOBALS['kernel']))->getTwig()->render('core/unauthorized.html.twig', ['pageTitle' => xl("Pending Followup")]);
    exit;
}
...
<title><?php echo xlt('Pending Followup') ?></title>
...
<h2><?php echo xlt('Pending Followup from Procedure Results')?></h2>

<form method='post' action='custom_pending_followup.php' onsubmit='return top.restoreSession()'>
...
if ($_POST['form_refresh'] || $_POST['form_csvexport']) {
    ...
    "pt.name AS procedure_name, pt.related_code, " .
    ...
    "JOIN procedure_providers AS pp ON po.lab_id = pp.ppid " .
    ...
    "JOIN procedure_type AS pt ON pt.procedure_code = poc.procedure_code and pt.lab_id = po.lab_id " .
//    "AND pts.related_code != '' " .
    "LEFT JOIN users AS u1 ON u1.id = po.provider_id " .
    "WHERE NULLIF(pp.type,'') IS NULL AND " .
