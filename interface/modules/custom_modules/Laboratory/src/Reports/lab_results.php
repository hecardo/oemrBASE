...
$form_provider = isset($_REQUEST['form_provider']) ? $_REQUEST['form_provider'] : null;
$form_facility = isset($_REQUEST['form_facility']) ? $_REQUEST['form_facility'] : null;
$form_status = isset($_REQUEST['form_status']) ? $_REQUEST['form_status'] : null;
$form_processor = isset($_REQUEST['form_processor']) ? $_REQUEST['form_processor'] : null;
$form_priority = isset($_REQUEST['form_priority']) ? $_REQUEST['form_priority'] : null;
$form_billing = isset($_REQUEST['form_billing']) ? $_REQUEST['form_billing'] : null;
$form_group = isset($_REQUEST['form_group']) ? $_REQUEST['form_group'] : null;
$form_results = isset($_REQUEST['form_results']) ? $_REQUEST['form_results'] : null;
...
try { // catch any page processing errors
	...
	$colDefs[] = array('orderable'=>false, 'targets'=>array(11));
	...
	if ($process == 'report' || $process == 'excel') {
		...
		// Global Search
		$sWhere = "WHERE (fo.`id` IS NOT NULL AND pr.`procedure_report_id` IS NOT NULL ";
		$sWhere .= "AND ( DATE(pr.`date_report`) >= ? AND DATE(pr.`date_report`) <= ? ) ";
		...
		if ($form_results == 'normal') {
			$sWhere .= ( empty($sWhere) ) ? "WHERE ( " : "AND ";
			$sWhere .= "fo.`request_abnormal` > 0 ";
		}
		if ($form_results == 'abnormal') {
			$sWhere .= ( empty($sWhere) ) ? "WHERE ( " : "AND ";
			$sWhere .= "(fo.`result_abnormal` = 0 OR fo.`result_abnormal` IS NULL) ";
		}
		...
		$sFrom = "
			...
			LEFT JOIN `procedure_report` pr ON pr.`procedure_order_id` = fo.`order_number`
		...
		$query = "
			SELECT u.`fname` AS doc_fname, u.`mname` AS doc_mname, u.`lname` AS doc_lname, pp.`name` AS 'lab_name',
				...
				ic.`name` AS 'insurance', pr.`date_report`
		";
				...
				foreach ($sColumns AS $column) {
					...
					switch ($column['name']) {
						...
						case 'ordered':
							...
							break;
						case 'reported':
							$time = strtotime($record['date_report']);
							$element = date('Y-m-d', $time);
							break;
						case 'patient':
...
<body class="body_top">
	<div id="container">
		...
		<header>
			<!-- HEADER (if desired) -->
			<span class="title" style="margin-left:10px">Laboratory - Results Report</span>
		</header>
...
?>
						...
						<div class="form-inline mr-4">
							<div class="control-label text-nowrap pr-2">Report Status:</div>
							<select class="form-control form-control-sm w-auto" name='form_status' id='form_status'>
								<option value=''>-- All --</option>
<?php
	// Build a drop-down list of form status.
	$query = "SELECT `option_id`, `title` FROM `list_options` WHERE `list_id` = 'Lab_Form_Status' AND `title` LIKE 'Result%' ORDER BY `seq`";
	$res = sqlStatement($query);
...
?>
						...
						<div class="form-inline mr-4">
							<div class="control-label text-nowrap pr-2">Report Start:</div>
							<input class="form-control form-control-sm w-auto" type='date' name='start_date' id='start_date'
									value='<?php echo $form_start ?>' />
						</div>
						<div class="form-inline mr-4">
							<div class="control-label text-nowrap pr-2">Report End:</div>
	...
	<script>
		...
		function editOrder(id) {
	        ...
	        dlgopen('', 'lab_order', 1200, 900, '', '', {
	            ...
	            dialogId: '',
	            type: 'iframe',
	            url: '<?php echo $web_root; ?>/interface/forms/laboratory/view.php?pop=1&id=' +id
	        });
			return false;
		}
